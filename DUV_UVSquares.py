...
precision = 3
blender5 = bpy.app.version >= (5, 0, 0)
    ...
    for f in bm.faces:
        ...
        for l in f.loops:
            if is_uv_vert_selected(l, uv_layer):
                face_edge_verts.append(l[uv_layer])
            else:
...
class DREAMUV_OT_uv_squares(bpy.types.Operator):
...
class DREAMUV_OT_uv_squares_by_shape(bpy.types.Operator):
...
class DREAMUV_OT_uv_rip_faces(bpy.types.Operator):
    """Rip UV faces apart"""
...
class DREAMUV_OT_uv_join_faces(bpy.types.Operator):
...
class DREAMUV_OT_uv_snap_to_axis(bpy.types.Operator):
